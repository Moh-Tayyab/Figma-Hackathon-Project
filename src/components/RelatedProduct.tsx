      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-center">
        ...
        <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          ...
          <div className="flex items-center gap-4 text-white">
            ...
            <WishListFunctionality product={relatedProduct} />
          </div>
        </div>
      </div>
        {/* Product Info */}
      <div className="p-4 pt-5">
        <Link href={`/shop/${relatedProduct.slug}`}>
          <h3 className="text-gray-800 font-semibold text-lg mb-2 hover:text-primary transition-colors line-clamp-2">
            {relatedProduct.slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
          </h3>
        </Link>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, index) => (
              <svg
                key={index}
                className={`w-4 h-4 ${index < Math.round(relatedProduct.rating.rate) ? 'text-amber-400' : 'text-gray-300'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
