...
import { cartAtom, customerFormDetails } from "@/lib/atom";
...
const OrderSuccessPage = () => {
  ...
  useEffect(() => {
    const SanatyUpload = async () => {
      try {
        const totalPriceCalc = cartItems.reduce((total: number, product: Product) => {
          return total + product.finalPrice * product.quantity;
        }, 0);

        console.log("Before calling saveOrderToSanity");

        await saveOrderToSanity (billingDetails, cartItems, totalPriceCalc);
...
function saveOrderToSanity(billingDetails: BillingDetails, cartItems: any, totalPriceCalc: any) {
